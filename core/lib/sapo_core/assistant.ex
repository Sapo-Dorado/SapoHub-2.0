defmodule SapoCore.Assistant do
  ...
  defp core_preamble do
    ...
    `sapo context`. For any other raw API calls, use `$SAPO_API_BASE/...`.
    When you finish a task or need user input, call
    `sapo notify "<short message>"` — suppression is handled automatically
    via SAPO_SESSION_ID.

    For `sapo` CLI usage, run `sapo help`, `sapo <resource> help`, or
    `sapo <resource>` with no action. `--help`/`-h`
