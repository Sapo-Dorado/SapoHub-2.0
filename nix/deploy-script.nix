...
let
  ...
  # module file (lib.mkDefault, so hand-written config always wins), at
  # the fixed, standardized location .sapohub/sapohub-prefs.nix — every
  # config repo's own modules list is expected to conditionally import
  # this exact path (see examples/user-config/flake.nix). After a
  # successful sync the overlay is consumed — the nix config now carries
  # the prefs.
  #
  ...
  PREFS_FILE=".sapohub/sapohub-prefs.nix"
  if [ "$SYNC_PREFS" = "1" ] && [ -f "$OVERLAY" ] && [ -s "$OVERLAY" ]; then
    echo "syncing UI preferences into $FLAKE_PATH/$PREFS_FILE ..."
    ...
    mkdir -p "$FLAKE_PATH/$(dirname "$PREFS_FILE")"
    cat > "$FLAKE_PATH/$PREFS_FILE" <<NIXEOF
  ...
  NIXEOF
    ...
    git -C "$FLAKE_PATH" add "$PREFS_FILE"
